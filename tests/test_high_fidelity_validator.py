...
class TestHighFidelityValidator(unittest.TestCase):
    ...
    def test_prepare_shortlist_validation_package_creates_su2_bundle(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn('顯示模式:', preview_html)
            self.assertIn('<option value="surface_grid">曲面+格線</option>', preview_html)
            ...
            self.assertIn('drawStructuredGrid(projected);', preview_html)
            ...
            self.assertEqual(geometry_payload["mesh"]["sectionCount"], 64)
            self.assertEqual(geometry_payload["mesh"]["sectionPoints"], 40)
